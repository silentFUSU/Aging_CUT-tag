...
tissues <- sort(c("BAT","mammarygland","CB","lung","aorta","spleen",
                  "thymus","skin","bladder","bonemarrow","Hip","heart",
                  "uterus","ovary","iWAT"))
to_plot <- data.frame()
for(tissue in tissues){
  opposite_change_region <- read.table(paste0("data/samples/",tissue,"/H3K27me3_H3K9me3_intersect/10kb_all_significant_second_quadrant.bed"))
  H3K9me3_peak_region <- read.table(paste0("data/samples/",tissue,"/H3K9me3/bed/H3K9me3_10kb_in_young_only-W1000-G3000-E100.bed"))
  opposite_change_region$condition <- "Out of H3K9me3 peaks"
  opposite_change_region$condition[which(opposite_change_region$V4 %in% H3K9me3_peak_region$V4)] <- "In H3K9me3 peaks"
...
############### H3K27me3 increase whether in H3K9me3 peaks
...
to_plot <- data.frame()
for(tissue in tissues){
  H3K27me3_increase_region <- read.csv(paste0("data/samples/",tissue,"/H3K27me3/H3K27me3_10kb_bins_diff.csv"))
  ...
  t_to_plot <- as.data.frame(table(H3K27me3_increase_region$condition))
  t_to_plot$percent <- t_to_plot$Freq/sum(t_to_plot$Freq) *100
  t_to_plot$tissue <- tissue_label_change(tissue)
  if(nrow(to_plot)==0){
    to_plot <- t_to_plot
...
for(tissue in tissues){
  ...
  H3K27me3_increase_region$condition <- "Out of H3K9me3 peaks"
  H3K27me3_increase_region$condition[which(H3K27me3_increase_region$Geneid %in% H3K9me3_peak_region$V4)] <- "In H3K9me3 peaks"
  H3K27me3_increase_region <- H3K27me3_increase_region[,c(paste(search_table$sample_name,search_table$age,search_table$mouse_ID,sep = "."),"condition","Geneid")]
  colnames(H3K27me3_increase_region)[which(colnames(H3K27me3_increase_region)!="condition")] <- sapply(strsplit(colnames(H3K27me3_increase_region)[which(colnames(H3K27me3_increase_region)!="condition")], "\\."), `[`, 1)
  H3K27me3_increase_region <- reshape2::melt(H3K27me3_increase_region)
  H3K27me3_increase_region <- merge(H3K27me3_increase_region,search_table,by.x="variable",by.y="sample_name")
  H3K27me3_increase_region$age <- factor(H3K27me3_increase_region$age, levels=c("young","old"))
  p_list[[tissue]] <- ggplot(H3K27me3_increase_region,aes(x=condition,y=log2(value),fill=age))+
    geom_boxplot()+
    xlab(NULL)+
...
ggplot(H3K27me3_increase_region,aes(x=condition,y=log2(value),fill=age))+
  geom_boxplot()+
  xlab(NULL)+
  ylab("H3K27me3 log2(CPM)")+
  theme_bw()+
...
for(tissue in tissues){
  ...
  if(nrow(H3K9me3[which(H3K9me3$Significant!="Stable"  & abs(H3K9me3$LogFC.old.young)>0.6),])>100){
    H3K9me3 <- H3K9me3[which(H3K9me3$Significant !="Stable" & abs(H3K9me3$LogFC.old.young) >0.6),]
...
for(tissue in tissues){
  ...
  H3K9me3_peak_region <- read.table(paste0("data/samples/",tissue,"/H3K9me3/bed/H3K9me3_10kb_in_young_old_merge-W1000-G3000-E100.bed"))
