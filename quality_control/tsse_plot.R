...
for(i in c(1:length(antibodys))){
  ...
  tsse$age[which(tsse$age=="3m")] <- "young"
  tsse$age[which(tsse$age=="24m")] <- "old"
  tsse$age <- factor(tsse$age, levels = c("young","old"))
  ggplot(tsse,aes(x=tissue,y=V3,color = age,shape=batch))+    
    geom_jitter(position = position_jitter(width = 0.2), size = 3, alpha = 0.7)+
    geom_text_repel(aes(label = batch), position = position_jitter(width = 0.2), size = 3) +
    ...
    xlab("")+labs(fill = "", color = "") +ylab("TSSe")
...
search_table_cut <- read.csv("data/samples/all/CUTTag_search_table_used_in_diff.csv")
