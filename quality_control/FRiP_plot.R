...
tissues <- c("brain","liver","testis","colon","kidney","lung","spleen","muscle","pancreas","Hip","cecum")
...
FRiP$tissue <- factor(FRiP$tissue,levels=c("brain_FC","brain_HIP","liver","testis","colon","kidney","lung","spleen","muscle","pancreas","Hip","cecum"))
for (i in c(1:length(antibodys))){
  antibody <- antibodys[i]
  ggplot(FRiP[which(FRiP$antibody==antibody),],aes(x=tissue,y=FRiP,fill = batch))+geom_boxplot()+
    scale_fill_brewer(palette="Set3")+
    ggtitle(paste0(antibody," FRiP"))+ylim(0,100)+
    theme_bw()+theme(text = element_text(size = 18))+xlab("")+labs(fill = "", color = "") +ylab("FRiP(%)")
  ggsave(paste0("result/all/QC/FRiP/",antibody,"_FRiP_rm_blacklist.png"),width = 15,height = 10)
  }
