...
# write.table(palette,"data/samples/30_distinct_color.txt",row.names = F,col.names = F)
...
decrease_p <- ggplot(decrease,mapping = aes(x=Freq,y=tissue,fill = tissue_legend))+
  ...
  theme(text = element_text(size = 18))+ scale_fill_manual(values = palette)+ xlim(0,(max(decrease$Freq,increase$Freq)+1000)) +guides(fill= guide_legend(title = ""))
