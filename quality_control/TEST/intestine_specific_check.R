...
correlation_plot <- function(antibody){
  search_table <- read.csv("data/samples/all/CUTTag_search_table.csv")
  if(antibody %in% c("H3K9me3","H3K36me3","H3K27me3")){
    bin_size <- "10kb"
  }else{
  ...
  rownames(annotation) <- paste0(search_table$tissue,"-",search_table$mouse_ID,"-",search_table$age)
  annotation <- annotation[,c("tissue","age")]
  annotation$age <- factor(annotation$age,c("3m","24m"))
  color <- read.table("data/samples/7_distinct_color.txt")
  tissue_color <- setNames(color$V1[1:4],unique(annotation$tissue))
  age_color <- setNames(c("red","blue"),c("3m","24m"))
  annotation_colors <- list(age=age_color,tissue=tissue_color)
  ...
  colnames(correlation_matrix) <- paste0(search_table$tissue,"-",search_table$mouse_ID,"-",search_table$age)
  colnames(correlation_matrix)[which(colnames(correlation_matrix)=="Cecum-110-24m")] <- "Cecum?Colon-110-24m"
  colnames(correlation_matrix)[which(colnames(correlation_matrix)=="Colon-110-24m")] <- "Colon?Cecum-110-24m"
...
correlation_plot("H3K36me3")

cpm_heatmap <- function(antibody){
  if(antibody %in% c("H3K9me3","H3K36me3","H3K27me3")){
    bin_size <- "10kb"
  }else{
    bin_size <- "1kb"
  ...
  annotation <- search_table[which(search_table$sample_name %in% colnames(logCPMs)),]
  rownames(annotation) <- paste0(search_table$tissue,"-",search_table$mouse_ID,"-",search_table$age)
  annotation <- annotation[,c("tissue","age")]
  annotation$age <- factor(annotation$age,c("3m","24m"))
  
  df <-counts[,c(1,3,5,7)]
  y= DGEList(counts=df,group=c("cecum","cecum","colon","colon"))
  keep = which(rowSums(cpm(y)>1)>=2)
  y = y[keep,]
  y <- calcNormFactors(y)
  ...
  search_table <- search_table[order(search_table$sample_name),]
  colnames(logCPMs) <- paste0(search_table$tissue,"-",search_table$mouse_ID,"-",search_table$age)
  colnames(logCPMs)[which(colnames(logCPMs)=="Cecum-110-24m")] <- "Cecum?Colon-110-24m"
  colnames(logCPMs)[which(colnames(logCPMs)=="Colon-110-24m")] <- "Colon?Cecum-110-24m"
  
  pheatmap::pheatmap(logCPMs,annotation = annotation, 
                     annotation_colors = annotation_colors,
                     show_rownames = F,scale = "row",main = paste0(antibody," Areas of difference in other three samples"))
