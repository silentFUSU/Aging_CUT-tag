...
mkdir ${data_path}${tissue}/TElocal/
...
TE_GTF_DICT=(  
  ["mm10"]="/storage/zhangyanxiaoLab/suzhuojie/ref_data/TE_reference/mm10_rmsk_TE.gtf.locInd"
) 
...
do
    echo TElocol -b ${data_path}${tissue}/bam/${sample}*.sorted.bam --sortByPos --mode multi --GTF $gene_gtf --TE $TE_gtf --project ${sample} --outdir ${data_path}${tissue}/TElocal/
    /storage/zhangyanxiaoLab/suzhuojie/software/TElocal/TElocal -b ${data_path}${tissue}/bam/${sample}*.sorted.bam --sortByPos --mode multi --verbose 3 \
        --GTF $gene_gtf --TE $TE_gtf --project ${data_path}${tissue}/TElocal/${sample} &
done
wait

len=$(find ${data_path}${tissue}/TElocal/ -type f -name "*.cntTable" | grep -v "combined.cntTable" | wc -l)  
counts=$(ls ${data_path}${tissue}/TElocal/*.cntTable | grep -v "combined.cntTable")
paste ${counts} | cut -f 1,$(seq -s, 2 2 $((2*len))) > ${data_path}${tissue}/TElocal/combined.cntTable