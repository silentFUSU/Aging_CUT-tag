...
diff_expression_analysis <- function(tissue){
  tab = read.delim(paste0("data/samples/RNA/",tissue,"/counts/",tissue,"_H3K9me3_peaks_all_regions.counts"),skip=1)
  ...
  label <- tab[,c(1:3)]
  label$Geneid <- paste0(label$Chr,":",label$Start,"-",label$End)
  label <- label[,"Geneid",drop=F]
  out <- merge(out,label,by="row.names")
  rownames(out) <- out$Row.names
  out <- out[,-1]
  write.csv(out,paste0("data/samples/RNA/",tissue,"/diff_expression_gene_change_in_H3K9me3_peaks_all_regions.csv"))
  out <- out[grepl("^peak", row.names(out)), ]
  ...
  ggsave(paste0("result/RNA/",tissue,"/relationship_gene_expression_in_H3K9me3_all_regions_with_H3K9me3_changes.png"),p,width=4,height=5,type="cairo")
}
