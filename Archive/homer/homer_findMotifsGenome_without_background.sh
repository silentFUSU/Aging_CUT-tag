...
do
    ...
    do
        bed=${data_path}motif_without_bg/bed/${tissue}_ATAC_peaks_diff_${condition}_homer.bed
        if [ -s "$bed" ]; then  
            while [ $(current_jobs) -ge $max_jobs ]; do  
                sleep 1  
            done  
            echo ${tissue} ${condition} begin
            mkdir ${data_path}motif_without_bg/${condition}/${tissue}
            findMotifsGenome.pl ${bed} mm10 ${data_path}motif_without_bg/${condition}/${tissue} -size 200 -mask & 
        fi
    done
done
wait
