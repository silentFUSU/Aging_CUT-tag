...
search_table <- search_table[which(search_table$tissue_label %in% tissues),]
search_table$tissue_label <- sapply(search_table$tissue,tissue_label_change)
...
# write.csv(search_table,"data/samples/sample_info/RNA.csv")
data_path <- read.table("data/samples/all/RNA_datapath.txt",skip = 1)
...
write.csv(sample_data_path,"data/samples/sample_info/RNA_data_path.csv",row.names = F)
