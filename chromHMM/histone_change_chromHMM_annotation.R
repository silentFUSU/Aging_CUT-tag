...
histone_change_region_chromHMM_annotation <- function(tissue,state_num,antibody){
  ...
  for(condition in conditions){
    histone_condition <- histone[which(histone$Significant==condition),c("Chr","Start","End")]
    if(nrow(histone_condition)>100){
      histone_condition$Start <- histone_condition$Start+1
      histone_condition <- as.data.table(histone_condition)
      setDT(histone_condition)  
      setkey(histone_condition, Chr, Start, End) 
      
      overlaps_young <- foverlaps(histone_condition, chromHMM_young, type = "any", nomatch = 0L)  
      overlaps_old <- foverlaps(histone_condition, chromHMM_old, type = "any", nomatch = 0L)  
