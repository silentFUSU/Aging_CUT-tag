...
write.csv(background_transfer_matrix,paste0("result/all/ChromHMM/all_tissues/",state_num,"_all_tissues/state_transfer/state_transfer_backgroud_cross_comparison.csv"))
...
for(i in c(1:length(tissues))){
  ...
  for(j in c(1:2)){
    t_young <- young[[j]]
    for(k in c(1:2)){
      tissue_transfer_matrix[[(j-1)*2 + k]] <- data.frame()
      t_old <- old[[k]]
      ...
      for(state in states){
        ...
        tissue_transfer_matrix[[(j-1)*2 + k]] <- rbind(tissue_transfer_matrix[[(j-1)*2 + k]],t_tissue_transfer_matrix)
      }
    }
  }
...
write.csv(transfer_matrix,paste0("result/all/ChromHMM/all_tissues/",state_num,"_all_tissues/state_transfer/state_transfer_cross_comparison.csv"))
...
#                    breaks = seq(0, 5, length.out = 101),
#                    fontsize = 10,na_col = "white",clustering_distance_rows = "manhattan",clustering_distance_cols = "manhattan",
