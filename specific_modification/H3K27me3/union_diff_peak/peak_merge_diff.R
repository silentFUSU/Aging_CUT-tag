...
peak_preprocess_peak_level_remove_batch_effect <- function(tissue,antibody){
  ...
  write.csv(out,paste0("data/samples/",tissue,"/",antibody,"/",antibody,"_young_merge-W5000-G10000-E100_peak_merged_exist_in_larger_",tissue_num,"_tissues_diff.csv"),row.names = F)
  ...
  # out <- out[which(out$Length <= 50000),]
  # p <- ggplot(
  #   out, aes(x = `LogFC.old-young`, y = -log10(`FDR.old-young`))) +
  #   geom_point(aes(color = Significant, size = Length),alpha=0.2) +
  #   scale_color_manual(values = colour) +
...
# ggsave(paste0("result/all/diff/",antibody,"/all_tissues_young_merge-W5000-G10000-E100_peak_merged_exist_in_all_tissues_volcano_plot_remove_batch_effect.png"),combined_plot,width = 40,height = 18,type="cairo")
