...
tissue <- "lung"
antibody <- "H3K27me3"
domain_preprocess_peak_level_remove_batch_effect <- function(tissue,antibody){
  search_table <- read.csv("data/samples/all/CUTTag_search_table_used_in_diff_batch.csv")
  tab = read.delim(paste0("data/samples/",tissue,"/",antibody,"/",antibody,"_edd_domain_merged.counts"),skip=1)
  ...
  write.csv(out,paste0("data/samples/",tissue,"/",antibody,"/",antibody,"_edd_domain_merged_diff_after_remove_batch_effect.csv"),row.names = F)
  ...
  out <- out[which(out$Length > 1000000),]
  p <- ggplot(
    out, aes(x = `LogFC.old-young`, y = -log10(`FDR.old-young`))) +
    geom_point(aes(color = Significant, size = Length),alpha=0.2) +
    scale_color_manual(values = colour) +
...
for(j in c(1:length(tissues))){
  tissue <- tissues[j]
  p_list[[j]] <- domain_preprocess_peak_level_remove_batch_effect(tissue,antibody)
}
combined_plot <- plot_a_list(p_list,4,7)
ggsave(paste0("result/all/diff/",antibody,"/all_tissues_edd_domain_merge_volcano_plot_remove_batch_effect.png"),combined_plot,width = 40,height = 18,type="cairo")
