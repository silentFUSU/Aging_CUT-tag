...
antibody <- "H3K27me3"
annotation <- read.csv("data/samples/all/H3K27me3/edd_domain_merged/kmeans_annotation.csv")
...
tissues <- c("aorta","BAT","bladder","bonemarrow","brain","CB","cecum","colon","heart","Hip","ileum","jejunum","kidney","liver","lung","pancreas","skin","spleen","stomach","testis","thymus","tongue","iWAT")
...
if(Indicator=="rank"){
  ...
  ggplot(to_plot,aes(x=histone,y=log2(value),color =variable))+    
    geom_jitter(size = 3, alpha = 0.7)+
    ggtitle(paste0("Gene expression positively correlated with the degree of upregulation of H3K27me3"))+
    theme_bw()+theme(text = element_text(size = 18),axis.text.x = element_text(angle = 45, hjust = 1))+
    xlab(Indicator)+labs(fill = "", color = "") +ylab(paste0("log2(CPM)"))
...
muscle_tissue_development <- read.csv("data/public_data/GO_term_summary_0060537.csv")
muscle_tissue_development <- unique(muscle_tissue_development$Symbol)
target_genes <- muscle_tissue_development
GO_id <- "GO:0140014"
description <- "muscle tissue development"
...
if(Indicator=="rank"){
  ...
  ggplot(to_plot,aes(x=histone,y=score,color = tissue,shape=age))+    
    ...
    ggtitle(paste0(description," with H3K27me3"))+
    theme_bw()+theme(text = element_text(size = 18),axis.text.x = element_text(angle = 45, hjust = 1))+
    xlab("H3K27me3 log2(Fold change)")+labs(fill = "", color = "")
}
