...
diff_expression_analysis <- function(tissue,antibody){
  ...
  if(antibody == "ATAC"){
    tab = read.delim(paste0("data/samples/",antibody,"/",tissue,"/",antibody,"/",tissue,"_H3K9me3_peaks.counts"),skip=1)
  }else{
    tab = read.delim(paste0("data/samples/",tissue,"/",antibody,"/",tissue,"_H3K9me3_peaks.counts"),skip=1)
  }

  if(tissue %in% c("mammarygland","ovary","uterus","MEF")){
    tab <- tab[which(tab$Chr %in% paste0("chr",c(1:19,"X"))),]
  }else{
    tab <- tab[which(tab$Chr %in% paste0("chr",c(1:19,"X","Y"))),]
  }
  ...
  if(antibody == "ATAC"){
    write.csv(out,paste0("data/samples/",antibody,"/",tissue,"/",antibody,"/",antibody,"_diff_in_H3K9me3_peaks_remove_batch_effect.csv"))
  }else{
...
tissues <- c("mammarygland","ovary","uterus")
# antibody <- "H3K4me3"
