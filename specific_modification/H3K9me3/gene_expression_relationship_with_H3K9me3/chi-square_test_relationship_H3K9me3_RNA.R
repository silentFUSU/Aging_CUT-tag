...
for(tissue in tissues){
  df <- read.csv(paste0("data/samples/",tissue,"/",antibody,"/",antibody,"_gene_TSS_diff_after_remove_batch_effect.csv"))
...
summary <- data.frame()
for(tissue in tissues){
  df <- read.csv(paste0("data/samples/",tissue,"/",antibody,"/",antibody,"_gene_TSS_100kb_diff_after_remove_batch_effect.csv"))
  rna <- read.csv(paste0("data/samples/RNA/",tissue,"/diff_expression_gene_change_filter_bar.csv"))
  colnames(rna)[1] <- "Geneid"
  to_plot <- merge(rna[,c("Geneid","logFC","Significant")],df[,c("Geneid","LogFC.old.young","Significant")],by="Geneid")
  histone_increase_rna_decrease <- nrow(to_plot[which(to_plot$Significant.x=="Down" & to_plot$Significant.y=="Up"),])
  ...
  t_summary <- data.frame(tissue=tissue_label_change(tissue),chi_pvalue=chi_test$p.value,residual=chi_test$stdres[2,1])
