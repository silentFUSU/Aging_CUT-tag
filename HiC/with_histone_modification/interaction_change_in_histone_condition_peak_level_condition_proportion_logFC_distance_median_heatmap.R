...
if(antibody=="H3K9me3"){
  p <- pheatmap::pheatmap(to_plot,breaks = breaks, color = color_palette,cluster_cols = F,main = antibody)
  tissue_order <- p[["gtable"]][["grobs"]][[5]][["label"]]
}else{
  to_plot <- to_plot[tissue_order,]
  pheatmap::pheatmap(to_plot,breaks = breaks, color = color_palette,cluster_cols = F,cluster_rows = F,main = antibody)
}
