...
tissues <- c("brain","CB","kidney","liver","lung","bonemarrow","colon","heart","Hip","mammarygland","stomach","thymus","skin","muscle","pancreas","spleen","ileum","cecum")
resolution <- "10000"
antibody <- "H3K27me3"
...
if(condition=="within-within"){
  p <- pheatmap::pheatmap(to_plot,breaks = breaks, color = color_palette,cluster_cols = F,main = paste0(antibody," domain"))
  tissue_order <- p[["gtable"]][["grobs"]][[5]][["label"]]
}else{
  to_plot <- to_plot[tissue_order,]
  pheatmap::pheatmap(to_plot,breaks = breaks, color = color_palette,cluster_cols = F,cluster_rows = F,main = paste0(antibody," domain"))
}
