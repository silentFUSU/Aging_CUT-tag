...
tissues <- c("brain","CB","kidney","liver","lung","bonemarrow","colon","heart","Hip","mammarygland","stomach","thymus","skin")
resolution <- "10000"
antibody <- "H3K9me3"
condition <- "out-out"
summary <- data.frame()
for(tissue in tissues){
  ...
  df <- df[which(df$condition==condition),]
  average_age_summary <- df %>%
...
pheatmap::pheatmap(to_plot,cluster_rows = F,breaks = breaks, color = color_palette,clustering_distance_cols = "manhattan",main = paste0(antibody," ",condition))
