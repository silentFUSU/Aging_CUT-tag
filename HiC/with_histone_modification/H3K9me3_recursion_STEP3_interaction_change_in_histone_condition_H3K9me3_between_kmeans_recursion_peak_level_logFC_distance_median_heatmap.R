...
tissues <- c("brain","CB","kidney","liver","lung","bonemarrow","colon","heart","Hip","mammarygland","stomach","thymus","skin","muscle","cecum","ileum","spleen","pancreas")
...
for(tissue in tissues){
  ...
  summary <- read.csv(paste0("result/HiC/",tissue,"/with_histone/H3K9me3_recursion_peaks/",resolution,"_interaction_relationship_with_",antibody,"_recursion_peaks_distance.csv"))  
...
other <- colnames(to_plot)[which(! colnames(to_plot) %in% self)]
to_plot <- to_plot[,c(self,other)]
p <- pheatmap::pheatmap(to_plot,breaks = breaks, border_color = "black",color = color_palette,cluster_cols = F,main = antibody,filename = "result/Sup_figures/HiC_H3K9me3_kmeans_peak_interaction.pdf",width = 8,height = 6)
