...
for(kmeans in paste0("kmeans",1:4)){
  df <- read.table(paste0("data/samples/all/H3K9me3/recursion_peaks_diff_table/bed/",kmeans,"_uinon_recursion_peaks.bed"))
  df$kmeans <- kmeans
  histone <- rbind(histone,df)
}
...
interaction_change_in_histone_condition_peak_level_logFC <- function(tissue,resolution){
  ...
  bed$condition[which(bed$V4 %in% overlaps$V4[which(overlaps$kmeans=="kmeans1")])] <- "kmeans1"
  bed$condition[which(bed$V4 %in% overlaps$V4[which(overlaps$kmeans=="kmeans2")])] <- "kmeans2"
  bed$condition[which(bed$V4 %in% overlaps$V4[which(overlaps$kmeans=="kmeans3")])] <- "kmeans3"
  bed$condition[which(bed$V4 %in% overlaps$V4[which(overlaps$kmeans=="kmeans4")])] <- "kmeans4"
  summary <- data.frame()
  ...
  CPM_average[, bin1_condition := ifelse(as.character(V1) %in% overlaps$V4[which(overlaps$kmeans=="kmeans1")], "kmeans1", bin1_condition)]
  CPM_average[, bin1_condition := ifelse(as.character(V1) %in% overlaps$V4[which(overlaps$kmeans=="kmeans2")], "kmeans2", bin1_condition)]
  CPM_average[, bin1_condition := ifelse(as.character(V1) %in% overlaps$V4[which(overlaps$kmeans=="kmeans3")], "kmeans3", bin1_condition)]
  CPM_average[, bin1_condition := ifelse(as.character(V1) %in% overlaps$V4[which(overlaps$kmeans=="kmeans4")], "kmeans4", bin1_condition)]
  
  CPM_average[, bin2_condition := ifelse(as.character(V2) %in% overlaps$V4[which(overlaps$kmeans=="kmeans1")], "kmeans1", bin2_condition)]
  CPM_average[, bin2_condition := ifelse(as.character(V2) %in% overlaps$V4[which(overlaps$kmeans=="kmeans2")], "kmeans2", bin2_condition)]
  CPM_average[, bin2_condition := ifelse(as.character(V2) %in% overlaps$V4[which(overlaps$kmeans=="kmeans3")], "kmeans3", bin2_condition)]
  CPM_average[, bin2_condition := ifelse(as.character(V2) %in% overlaps$V4[which(overlaps$kmeans=="kmeans4")], "kmeans4", bin2_condition)]
  
  CPM_average[, combined_condition := paste(bin1_condition, bin2_condition, sep = "-")]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans2-kmeans1", "kmeans1-kmeans2", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans3-kmeans1", "kmeans1-kmeans3", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans3-kmeans2", "kmeans2-kmeans3", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans4-kmeans1", "kmeans1-kmeans4", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans4-kmeans2", "kmeans2-kmeans4", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans4-kmeans3", "kmeans3-kmeans4", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans1-out", "out-kmeans1", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans2-out", "out-kmeans2", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans3-out", "out-kmeans3", combined_condition)]
  CPM_average[, combined_condition := ifelse(combined_condition == "kmeans4-out", "out-kmeans4", combined_condition)]
  ...
  if(resolution == "200000"){
    ...
    write.csv(median_values,paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_between_kmeans_recursion_peak_level_logFC.csv"),row.names = F)
  }else{
    mean_values <- to_plot %>%
      filter(is.finite(log2_ratio)) %>%
      group_by(combined_condition) %>%
      summarise(mean_log2_ratio = mean(log2_ratio, na.rm = TRUE))
    write.csv(mean_values,paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_between_kmeans_recursion_peak_level_logFC.csv"),row.names = F)
  }
...
to_plot <- data.frame()
for(tissue in tissues){
  df <- read.csv(paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_between_kmeans_recursion_peak_level_logFC.csv"))
  colnames(df)[2] <- tissue_label_change(tissue)
  if(nrow(to_plot) ==0){
    to_plot <- df
  }else{
    to_plot <- merge(to_plot,df,by="combined_condition")
  }
}
color_palette <- colorRampPalette(c("blue", "white", "red"))(100)
breaks <- c(seq(-0.2, -0.06, length.out = 40), seq(-0.05, 0.05, length.out = 20), seq(0.06, 0.2, length.out = 40))
rownames(to_plot) <- to_plot$combined_condition
to_plot <- to_plot[,-1]
pheatmap::pheatmap(to_plot,breaks = breaks,color = color_palette,cluster_cols = T,cluster_rows = F)
