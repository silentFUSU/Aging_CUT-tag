...
for(tissue in tissues){
  ...
  tab <- read.table(paste0("data/samples/RNA/",tissue,"/counts/",tissue,"_compartment_50000.counts"),header = T)
  tab_summary <- read.table(paste0("data/samples/RNA/",tissue,"/counts/",tissue,"_compartment_50000.counts.summary"),header = T)
...
to_plot <- to_plot[which(to_plot$condition %in% c("A-B","B-A")),]
to_plot$condition <- factor(to_plot$condition,levels = c("A-A","A-B","B-B","B-A"))
color <- setNames(c("#f39b7f","#4dbbd5"),c("young","old"))
p <- ggplot(to_plot, aes(x = condition, y = mean_rowmeans,fill=age)) +
...
ggsave("result/Sup_figures/compartment_change_RNA_change.pdf",p,width = 6,height = 8)
