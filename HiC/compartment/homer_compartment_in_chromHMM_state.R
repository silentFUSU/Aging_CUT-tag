...
compartment_change_in_chromHMM_state <- function(tissue,state_num,resolution){
  ...
  compartment <- compartment[,-1]
  color_palette <- colorRampPalette(c("white", "blue"))(101) 
  pheatmap::pheatmap(compartment,cluster_rows = F,cluster_cols = F,breaks = seq(0, 100, length.out=101),color = color_palette,main = paste(tissue_label_change(tissue),"compartment change"),display_numbers = T)
  }