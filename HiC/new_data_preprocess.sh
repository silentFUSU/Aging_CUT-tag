raw_data=/storage/zhangyanxiaoLab/suzhuojie/projects/Aging_CUT_Tag/data/raw_data/20241127_DYQ_HiC/
...
do
    ...
    do
        if ls ${raw_data}result/hic_results/data/${sample_for_tissue}/${sample_for_tissue}*.allValidPairs 1> /dev/null 2>&1; then  
            mv ${raw_data}result/hic_results/data/${sample_for_tissue}/${sample_for_tissue}*.allValidPairs ${data_path}${tissue}/ValidPairs/
        fi
        if ls ${raw_data}result/hic_results/data/${sample_for_tissue}*.allValidPairs.hic 1> /dev/null 2>&1; then  
            mv ${raw_data}result/hic_results/data/${sample_for_tissue}*.allValidPairs.hic ${data_path}${tissue}/juicer/
        fi
    done
done