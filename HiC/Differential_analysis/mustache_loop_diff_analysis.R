...
loop_diff_analysis <- function(tissue,resolution){
  ...
  loop <- read.table(paste0("data/samples/HiC/",tissue,"/loop/mustache/merged_",resolution_label,"_loop.bed"),skip = 1,header = F)
  loop <- loop[,c(1:6)]
  ...
  ggsave(paste0("result/HiC/",tissue,"/differential_analysis/",tissue,"_mustache_Loop_",resolution,"_diff_volcano_edger.png"),p,width = 5,height = 6,type="cairo")
}
