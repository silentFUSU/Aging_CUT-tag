...
loop_diff_analysis <- function(tissue,resolution){
  loop <- read.table(paste0("data/samples/HiC/",tissue,"/loop/homer/merged_",resolution,".loop.2D.bed"),skip = 1,header = F)
  loop <- loop[,c(1:6)]
  ...
  ggsave(paste0("result/HiC/",tissue,"/differential_analysis/",tissue,"_Homer_Loop_20000_diff_volcano_edger.png"),p,width = 5,height = 6,type="cairo")
}
