...
for(tissue in tissues){
  df <- read.table(paste0("data/samples/",tissue,"/",antibody,"/peaks/edd/edd_peaks_fdr05.bed"))
...
p <- ggplot(to_plot, aes(x = age, y = mean, fill = Var1)) +  
  ...
  ylab("Proportion")
ggsave("result/Sup_figures/H3K27me3_domain_chromHMM_state.pdf",p,width = 6,height = 8)
