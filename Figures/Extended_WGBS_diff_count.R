...
ggsave(paste0("result/figures/all_tissues_DMR_after_remove_batch_effect.pdf"), plot = p, width = 4, height = 6)
...
for(i in c(1:length(conditions))){
  ...
  combined_plot <- arrangeGrob(  
    ...
    top = textGrob(paste0("DMR number"), gp = gpar(fontsize = 15, fontface = "bold"))  
  )  
  ggsave(paste0("result/Sup_figures/WGBS_minCG3_delta01.pdf"), plot = combined_plot, width = 6, height = 5)
