...
for(kmean in c(paste0("kmeans",1:4),"Stable")){
...
dictionary <- list("state1"=1, "state2"=2, "state3"=3,
                   "state4"=4, "state5"=5, "state6"=7,
                   "state7"=8, "state8"=6, "state9"=9,
                   "state10"=10,"state11"=11,"state12"=15,
                   "state13"=12,"state14"=13,"state15"=14)
keys <- names(dictionary)
values <- unlist(dictionary)
to_plot$State_label <- values[match(to_plot$State, keys)]
to_plot$State <- paste0("state",to_plot$State_label)
...
p<-ggplot(to_plot, aes(x = variable, y = value, fill = State)) +  
  ...
  ylab("Proportion")
ggsave("result/Sup_figures/H3K9me3_kmeans_chromHMM_old.pdf",p,width = 6,height = 8)
