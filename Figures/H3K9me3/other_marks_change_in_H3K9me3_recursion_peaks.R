...
logFC_heatmap_all_tissues <- function(antibody){
  ...
  for(tissue in tissues){
    if(antibody == "H3K9me3"){
      ...
      histone <- read.csv(paste0("data/samples/RNA/",tissue,"/diff_expression_gene_change_in_H3K9me3_peaks.csv")) 
      colnames(histone)[1] <- "Geneid"
    }else{
      histone <- read.csv(paste0("data/samples/",tissue,"/",antibody,"/",antibody,"_diff_in_H3K9me3_peaks_remove_batch_effect.csv"))
    }
    if(antibody == "RNA"){
      histone <- histone[which(histone$Geneid %in% H3K9me3$X), c("Geneid","logFC")]
    }else{
