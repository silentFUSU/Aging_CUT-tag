...
for ((i=0; i<${#IP_samples[@]}; i++)); do  
    ...
    # bamCompare -b1 ${data_path}${IP_samples[i]}/${IP_samples[i]}.bam -b2 ${data_path}${input_samples[i]}/${input_samples[i]}.bam --operation ratio -normalizeUsingRPKM \
    #     -bs 50 -p 16 -o ${data_path}bw/${IP_samples[i]}_RPKM_ratio.bw 
    bamCoverage -b ${data_path}${IP_samples[i]}/${IP_samples[i]}.bam -o ${data_path}bw/${IP_samples[i]}_IP.bw  --outFileFormat bigwig -bs 50 --numberOfProcessors 16 --normalizeUsing RPKM &
    bamCoverage -b ${data_path}${input_samples[i]}/${input_samples[i]}.bam -o ${data_path}bw/${input_samples[i]}_input.bw  --outFileFormat bigwig -bs 50 --numberOfProcessors 16 --normalizeUsing RPKM &
    wait
done   