...
samples <- c("G1","G2","DS1","DS2")
for(i in c(1:length(samples))){
  sample <- samples[i]
  chr_list <- list()
  for(chr in chromosomes){
    df <- read.table(paste0("data/public_data/WANG_cellular_aging_HiC/TAD/insulation_score/",sample,"/",sample,"_",resolution,"_",chr,"_dense.is500001.ids200001.insulation.boundaries.bed"),skip=1)
  ...
  write.csv(tads,paste0("data/public_data/WANG_cellular_aging_HiC/TAD/insulation_score/",sample,"/",sample,"_",resolution,"_tads.csv"),row.names = F)
}
