...
peak_preprocess_bin_level <- function(tissue,antibody){
  search_table <- read.csv("data/public_data/cellular_aging_GSE133292/Chip_seq/CUTTag_search_table_used_in_diff_batch.csv")
  if(antibody %in% c("H3K27me3","H3K9me3","H3K36me3")){
    bin_size <- "10kb"
  }else{
    bin_size <- "1kb"
  }
  tab = read.delim(paste0("data/public_data/cellular_aging_GSE133292/Chip_seq/",antibody,"_",bin_size,"_bins.counts"),skip=1)
  ...
  out$Significant <- ifelse(out$`FDR.old-young` < 0.05 & abs(out$`LogFC.old-young`) >= log2(2), 
                            ifelse(out$`LogFC.old-young` > log2(2), "Up", "Down"), "Stable")
  out_sort <- out[order(out$`FDR.old-young`),]
  # dir.create(paste0("data/samples/all/diff_table/",antibody))
  write.csv(out_sort,paste0("data/public_data/cellular_aging_GSE133292/Chip_seq/",antibody,"_",bin_size,"_bins_diff.csv"),row.names = F)
