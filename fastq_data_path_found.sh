...
data=/storage/zhangyanxiaoLab/suzhuojie/projects/Aging_CUT_Tag/data/samples/all/tmp_map_dup_qc.csv
cleaned_file=$(mktemp)
cat "$data" | tr -d '\r' | awk '{gsub(/[\x00-\x1F\x7F]+/, ""); print}' > "$cleaned_file"  

output_file="/storage/zhangyanxiaoLab/suzhuojie/projects/Aging_CUT_Tag/data/samples/all/tmp_CUTTAG_ATAC_datapath.txt"

# Clear the output file if it exists
> "$output_file"

while IFS=$',' read -r sampleID tissue map dup antibody data_path; do
  # Skip header line
