...
for(antibody in antibodys){
  for(condition in conditions){
    ...
    write.csv(out,paste0("data/samples/MEF_OE/",antibody,"/",condition,"/",antibody,"_",condition,"_gene_TSS_10kb_spike_in_diff_after_remove_batch_effect.csv"),row.names = F)
    ...
    if(length(second) >0){
      ...
      second_GO <- enrichGO( second$ENTREZID,#GO富集分析
                             ...
                             readable = T)
      write.csv(second_GO@result,paste0("result/MEF_OE/",condition,"_OE_",antibody,"_RNA_second_spike_in.csv"))
      if(nrow(second_GO@result) >0){
        ...
        second_p_list[[paste0(antibody,"_",condition,"_second")]] <- NULL
      }
    }else{
      second_p_list[[paste0(antibody,"_",condition,"_second")]] <- NULL
    }
    ...
    if(length(fourth) > 0){
      ...
      if(nrow(fourth_GO) > 0){
        ...
        fourth_p_list[[paste0(antibody,"_",condition,"_fourth")]] <- NULL
      }
    }else{
      fourth_p_list[[paste0(antibody,"_",condition,"_fourth")]] <- NULL
    }
...
# combined_p
ggsave("result/MEF_OE/histone_change_RNA_spike_in.png",combined_p,width=15,height=10)
...
ggsave("result/MEF_OE/histone_change_RNA_GO_H3K27me3_spike_in.png",combined_p,width=30,height=15)
...
combined_p <- plot_a_list(GO_p_list,no_of_rows=2,no_of_cols=3)
ggsave("result/MEF_OE/histone_change_RNA_GO_H2AK119ub1_spike_in.png",combined_p,width=30,height=15)
