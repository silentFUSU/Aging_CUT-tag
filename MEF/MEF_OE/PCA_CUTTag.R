...
for(antibody in antibodys){
  for(condition in conditions){
    tab <- read.table(paste0("data/samples/MEF_OE/",antibody,"/",antibody,"_MEF_Vector_",condition,"_10kb.counts"),header = T)
    ...
    p_list[[paste0(condition,"-",antibody)]]<- ggplot(to_plot, aes(x=PC1, y=PC2, color=age)) + 
