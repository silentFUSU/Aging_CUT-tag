...
# peaks <- read.table("data/samples/MEF/H3K9me3/bed/H3K9me3_young_merge-W5000-G10000-E100.bed")
# peaks <- read.table("data/public_data/Bmi1_GSE112460/peaks/Bmi1_peaks.bed")
# peaks <- read.table("data/public_data/Cbx7_GSE151901/peaks/Cbx7_peaks.bed")
...
for(condition in conditions){
  ...
  color <- setNames(c("#f39b7f","#3c5488","#4dbbd5","#e64b35"),c("first","second","third","fourth"))
  p_list[[condition]] <- ggplot(to_plot[which(to_plot$condition=="Stable"),], aes(x = `LogFC.oe.vec`, y = `LogFC.old.young`)) +
    geom_point(alpha=0.1,color="dark grey") +
    geom_point(data = to_plot[which(to_plot$condition!="Stable"),], aes(x = `LogFC.oe.vec`, y = `LogFC.old.young`,color=quadrant)) +
    ...
    # ggsave(paste0("result/figures/MEF_OE_",condition,"_H3K27me3_correlation_with_senescence.png"),p_list[[condition]],width = 6,height = 6)
    ...
    p_list2[[condition]] <- ggplot(to_plot_table, aes(x = Var2, y = prop, fill = Var1)) +
      ...
      theme_bw()+ggtitle(condition)
    
    for(quadrant in c("first","second","third","fourth")){
      bin <- to_plot[which(to_plot$quadrant==quadrant),c("Chr","Start","End")]
      bin <- bin[order(bin$Chr,bin$Start),]
      write.table(bin,paste0("data/samples/MEF_OE/H3K27me3/",condition,"/bed/H3K27me3_correlation_with_senescence_10kb_bin_",quadrant,".bed"),quote = F,append = F,sep = "\t",row.names = F,col.names = F)
    }
    }
...
ggsave("result/MEF_OE/H3K27me3_bin_corr_with_senescence.png",combined_p,width=18,height=5)

combined_p <- plot_a_list(p_list2,no_of_rows=1,no_of_cols=3)
ggsave("result/MEF_OE/H3K27me3_bin_corr_with_senescence_overlap_Cbx7_peaks.png",combined_p,width=18,height=5)

color <- setNames(c("dark grey","#3c5488","dark grey","#e64b35","dark grey"),c("first","second","third","fourth","other"))
p <- ggplot(to_plot[to_plot$condition=="Stable", ],
                              aes(x = `LogFC.oe.vec`, y = `LogFC.old.young`)) +
  geom_point(color = "dark grey") +
  geom_point(data = to_plot[to_plot$condition!="Stable", ],
             aes(color = quadrant)) +
  scale_color_manual(values = color) +
...
ggsave(paste0("result/figures/MEF_OE_",condition,"_H3K27me3_correlation_with_senescence3.png"),p,width = 6,height = 6)

p <- ggplot(to_plot_table[which(to_plot_table$Var2!="other"),], aes(x = prop, y = Var2, fill = Var1)) +
  geom_col(position = "stack", width = 0.8) +
  labs(x = "Var2", y = "Proportion", fill = "Var1") +
  theme_bw()+ggtitle(condition)
p
ggsave(paste0("result/figures/MEF_OE_",condition,"_H3K27me3_correlation_with_senescence_overlap_in_p2_H3K9me3_peaks.pdf"),p,width = 6,height = 4)

color <- setNames(c("dark grey","#3c5488","dark grey","#e64b35","dark grey"),c("first","second","third","fourth","other"))
p <- ggplot(to_plot[to_plot$condition!="Stable", ],
       aes(x = `LogFC.oe.vec`, y = `LogFC.old.young`,color=quadrant)) +
  geom_point() +
  scale_color_manual(values = color) +
...
ggsave(paste0("result/figures/MEF_OE_",condition,"_H3K27me3_correlation_with_senescence2.png"),p,width = 6,height = 6)
