...
for(antibody in antibodys){
  for(condition in conditions){
    spike_in <- read.table("data/raw_data_transposon2/20260211_DYQ_CUTTag_SAMPLE/all_sample.qc.txt",header = T)
    spike_in <- spike_in[order(spike_in$Sample),]
    spike_in$Sample <- sub("_.*$", "", spike_in$Sample)
    search_table <- read.csv("data/samples/all/CUTTag_search_table_used_in_diff_batch.csv")
    tab = read.delim(paste0("data/samples/MEF_OE/",antibody,"/",antibody,"_MEF_Vector_",condition,"_merge-W5000-G10000-E100.counts"),skip=1)
    ...
    spike_in <- spike_in[spike_in$Sample %in% colnames(counts),]
    spike_in$Sample <- factor(spike_in$Sample,levels=colnames(counts))
    spike_in <- spike_in[order(spike_in$Sample),]
    ...
    pref <- median(p)
    s <- p / pref 
    y$samples$lib.size <- spike_in$Filtered
    y$samples$lib.size <- y$samples$lib.size / s
    ...
    lrt <- glmQLFTest(fit, coef=which(colnames(design)=="yearoe"))
    tab<-tab[keep,]
    
    out = cbind(tab[,1:6],cpm(y),logCPM=lrt$table$logCPM,bcv=sqrt(fit$dispersion),
    ...
    write.csv(out,paste0("data/samples/MEF_OE/",antibody,"/",condition,"/",antibody,"_",condition,"_peaks_spike_in_diff_after_remove_batch_effect.csv"),row.names = F)
    write.table(out[which(out$Significant=="Up"),c(2:4)],paste0("data/samples/MEF_OE/",antibody,"/",condition,"/bed/peaks_up_spike_in.bed"),append = F,quote = F,sep = "\t",row.names = F,col.names = F)
    write.table(out[which(out$Significant=="Down"),c(2:4)],paste0("data/samples/MEF_OE/",antibody,"/",condition,"/bed/peaks_down_spike_in.bed"),append = F,quote = F,sep = "\t",row.names = F,col.names = F)
...
# combined_p
ggsave("result/MEF_OE/all_diff_peak_spike_in.png",combined_p,width=15,height=10)
