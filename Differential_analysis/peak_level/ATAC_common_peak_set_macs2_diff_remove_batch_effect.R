...
peak_preprocess_peak_level_remove_batch_effect <- function(tissue){
  ...
  tissue_peaks <- read.table(paste0("data/samples/ATAC/",tissue,"/ATAC/bed/ATAC_macs_young_old_narrowpeak_summits_spm3.bed")) 
  tissue_peaks <- as.data.table(tissue_peaks[,c(1:3)])
  setDT(tissue_peaks)
  setkey(tissue_peaks,V1,V2,V3)
  common_peaks <- as.data.table(tab[,c("Geneid","Chr","Start","End")])
  setDT(common_peaks)
  setkey(common_peaks,Chr,Start,End)
  overlaps <- foverlaps(common_peaks,tissue_peaks, type = "any", nomatch = 0L)
  ...
  write.csv(out,paste0("data/samples/ATAC/",tissue,"/ATAC/ATAC_macs_young_old_narrowpeak_summits_spm3_all_tissues_merge_diff_after_remove_batch_effect.csv"),row.names = F)
}
