...
peak_preprocess_bin_level <- function(tissue,antibody){
  ...
  if (tissue=="ovary"){
    ...
    colnames(counts) = c("old_1","young_1","young_2","old_2")
    group =c("old","young","young","old")
  }else if(tissue == "cecum"){
    colnames(counts) = c("young_1","young_2","old_2","old_1")
...
tissues <- c("brain","liver","testis","kidney","lung","spleen","muscle","pancreas","Hip","bonemarrow","heart","thymus","stomach","skin","aorta","tongue","bladder","CB","jejunum","uterus","ovary")
antibodys <- c("H3K27me3","H3K36me3","H3K4me3","H3K4me1","H3K27ac")
p_list <- list()
for (i in c(1:length(tissues))){
  tissue <- tissues[i]
  for(j in c(1:length(antibodys))){
    antibody <- antibodys[j]
    p_list[[j]] <- peak_preprocess_bin_level(tissue,antibody)
  }
  combined_plot <- plot_a_list(p_list,2,3)
  ggsave(paste0("result/",tissue,"/all_diff_volcano_plot.png"),combined_plot,width = 20,height = 10,type="cairo")
}
...
for(i in c(1:length(antibodys))){
  ...
  ggsave(paste0("result/all/diff/",antibody,"/all_tissues_diff_volcano_plot.png"),combined_plot,width = 28,height = 10,type="cairo")
}
...
combined_plot <- plot_a_list(p_list,2,3)
ggsave(paste0("result/",tissue,"/all_diff_volcano_plot_remove_batch_effect.png"),combined_plot,width = 20,height = 10,type="cairo")
