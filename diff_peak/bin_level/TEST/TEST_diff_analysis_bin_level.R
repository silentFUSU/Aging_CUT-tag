...
peak_preprocess_bin_level <- function(tissue,antibody){
  ...
  write.csv(out_sort,paste0("data/samples/all/diff_table/",antibody,"/",tissue,"_",antibody,"_",bin_size,"_bins_diff.csv"),row.names = F)
...
for(tissue in tissues){
  for(antibody in antibodys){
    peak_preprocess_bin_level(tissue,antibody)
  }
}
for (i in c(1:length(tissues))){
  ...
  ggsave(paste0("result/",tissue,"/all_diff_volcano_plot.png"),combined_plot,width = 15,height = 10,type="cairo")
}
