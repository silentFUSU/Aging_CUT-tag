...
regions <- read.csv("data/samples/all/H3K9me3/recursion_peaks_diff_table/kmeans_annotation_add_stable.csv")
...
for(tissue in tissues){
  ...
  search_table <- search_table[which(search_table$tissue==tissue & search_table$antibody==antibody & search_table$age==age),]
...
rpkm_summary <- rpkm_summary[annotation$label,]
...
rpkm_summary <- rpkm_summary[annotation$label,tissues_order]
...
breaks <- c(seq(0, 5, length.out = 100))
...
pheatmap::pheatmap(rpkm_summary,cluster_rows = F,cluster_cols = F,main=paste(antibody,age),annotation_row = annotation,breaks = breaks,show_rownames = F,color = color_palette,annotation_colors = annotation_color)
