...
diff_expression_analysis <- function(tissue){
  ...
  tab = read.delim(paste0("data/samples/ATAC/",tissue,"/ATAC/",tissue,"_H3K9me3_peaks.counts"),skip=1)
  ...
  write.csv(out,paste0("data/samples/ATAC/",tissue,"/ATAC/ATAC_diff_in_H3K9me3_peaks_remove_batch_effect.csv"))
}
...
ggplot(decrease_summary, aes(x = tissue, y = logFC, fill=tissue)) +  
  ...
  ylab(paste0("ATAC log2(Fold change)")) +
  xlab(NULL)+
...
ggplot(merged_data, aes(x = Type, y = Tissue, fill = Value)) +
  ...
  ggtitle(paste0("ATAC log2(Fold change)"))+
