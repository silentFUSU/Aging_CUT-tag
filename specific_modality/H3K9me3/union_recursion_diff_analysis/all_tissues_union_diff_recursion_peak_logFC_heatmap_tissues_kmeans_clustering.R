...
color_palette <- colorRampPalette(c("blue", "white", "red"))(100)
breaks <- c(seq(-1, -0.29, length.out = 40), seq(-0.3, 0.3, length.out = 20), seq(0.31, 1, length.out = 40))
annotation_color <- list(cluster=setNames(c("#f6416c", "#f8f3d4", "#ffde7d", "#00b8a9"),c(1:4)))
pheatmap::pheatmap(data_for_heatmap,cluster_rows = F,show_rownames = F,
                   ...
                   color = color_palette, clustering_distance_cols="manhattan",clustering_method = "average",
                   border_color = "black",gaps_row = c(283,283+271,283+271+347,283+271+347+187),
                   width = 6,height =9)
...
kmeans_result_tissue <- kmeans_result_tissue[order(kmeans_result_tissue$`kmeans_result_tissue$cluster`),,drop=F]
