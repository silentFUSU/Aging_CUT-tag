...
pheatmap::pheatmap(to_plot,cluster_rows = F,show_rownames = F,
                   ...
                   border_color = "black",gaps_row = c(283,283+271,283+271+347,283+271+347+187),cluster_cols = F,
                   width = 6,height =9)
to_plot <- merge(to_plot,annotation,by="row.names")
to_plot$cluster[which(to_plot$cluster %in% c(1:4))] <- paste0("K",to_plot$cluster[which(to_plot$cluster %in% c(1:4))])
to_plot <- to_plot[order(to_plot$cluster),]
write.csv(to_plot,"data/samples/sample_info/H3K9me3_heatmap.csv",row.names = F)
