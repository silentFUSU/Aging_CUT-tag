...
antibody <- "H3K9me3"
regions <- read.csv("data/samples/all/H3K9me3/recursion_peaks_diff_table/kmeans_annotation.csv")
regions <- regions[!grepl("^chrY", regions$X), ]
regions <- regions$X
tissues <- c("aorta","BAT","bladder","bonemarrow","brain","CB","cecum","colon","heart","Hip","ileum","jejunum","kidney","liver",
             "lung","muscle","pancreas","skin","spleen","stomach","testis","thymus","tongue","iWAT")
...
summary <- apply(diff_summary, 2, median, na.rm = TRUE)
summary <- data.frame(tissues=colnames(diff_summary),logFC=summary)
summary <- summary[order(summary$logFC),]
summary$rank <- 1:nrow(summary)
colnames(summary) <- c("tissue","logFC","rank")
counts <- data.frame() 
