...
for(tissue in tissues){
  ...
  peak_region <- read.table(paste0("data/samples/",tissue,"/",antibody,"/bed/",antibody,"_young_merge-W5000-G10000-E100.bed"))
  peak_region <- as.data.table(peak_region[,c(1:3)])
  setDT(peak_region)
  setkey(peak_region,V1,V2,V3)
  
  bin <- as.data.table(opposite_change_region)
  setDT(bin)
  setkey(bin,V1,V2,V3)
  overlap <- foverlaps(bin, peak_region, type = "any", nomatch = 0L)  
  
  opposite_change_region$condition <- paste0("Out of ",antibody," peaks")
  opposite_change_region$condition[which(opposite_change_region$V4 %in% overlap$V4)] <- paste0("In ",antibody," peaks")
...
to_plot_peaks <- to_plot[which(to_plot$Var1==paste0("In ",antibody," peaks")),]
...
color <- setNames(c("#009980","#838B8B"),c(paste0("In ",antibody," peaks"),paste0("Out of ",antibody," peaks")))
p <- ggplot(to_plot, aes(x = tissue, y = percent, fill = Var1)) +  
...
ggsave(paste0("result/Sup_figures/H3K27me3_increase_H3K9me3_decrease_bin_in_",antibody,"_peaks.pdf"),p,width = 8,height = 6)
...
for(tissue in tissues){
  ...
  H3K27me3_change_region$Start <- H3K27me3_change_region$Start +1
  H3K27me3_change_region <- H3K27me3_change_region[which(H3K27me3_change_region$Significant==condition),]
  H3K9me3_peak_region <- read.table(paste0("data/samples/",tissue,"/H3K9me3/bed/H3K9me3_young_merge-W5000-G10000-E100.bed"))
  H3K9me3_peak_region <- as.data.table(H3K9me3_peak_region[,c(1:3)])
  setDT(H3K9me3_peak_region)
  setkey(H3K9me3_peak_region,V1,V2,V3)
  bin <- as.data.table(H3K27me3_change_region[,c(1:4)])
  setDT(bin)
  setkey(bin,Chr,Start,End)
  overlap <- foverlaps(bin, H3K9me3_peak_region, type = "any", nomatch = 0L)  
  
  H3K27me3_change_region$condition <- "Out of H3K9me3 peaks"
  H3K27me3_change_region$condition[which(H3K27me3_change_region$Geneid %in% overlap$Geneid)] <- "In H3K9me3 peaks"
...
ggsave("result/Sup_figures/H3K27me3_increase_bin_in_H3K9me3_peaks.pdf",p,width = 10,height = 6)
...
to_plot_sort <- to_plot[which(to_plot$Var1 == "H3K9me3 decrease"),]
to_plot_sort <- to_plot_sort[order(to_plot_sort$Freq),]
to_plot$tissue <- factor(to_plot$tissue,levels = to_plot_sort$tissue)
p <- ggplot(to_plot, aes(x = tissue, y = percent, fill = Var1)) +  
  geom_bar(stat = 'identity',colour = "white") +   
  theme_minimal() +   
  scale_fill_manual(values = color) +
...
ggsave("result/Sup_figures/H3K27me3_increase_how_H3K9me3_change.pdf",p,width = 10,height = 6)
...
to_plot_sort <- to_plot[which(to_plot$Var1=="H3K27me3 increase"),]
to_plot_sort <- to_plot_sort[order(to_plot_sort$Freq),]
to_plot$tissue <- factor(to_plot$tissue,levels=to_plot_sort$tissue)
p <- ggplot(to_plot, aes(x = tissue, y = percent, fill = Var1)) +  
  geom_bar(stat = 'identity',colour = "white") +   
  ...
  ylab("Proportion")+
  ggtitle("H3K9me3 decrease regions")
ggsave("result/Sup_figures/H3K9me3_decrease_how_H3K27me3_change.pdf",p,width = 10,height = 6)
...
for(tissue in tissues){
  ...
  H3K9me3_peak_region <- read.table(paste0("data/samples/",tissue,"/H3K9me3/bed/H3K9me3_10kb_in_young_merge-W5000-G10000-E100.bed"))
...
ggsave("result/Sup_figures/H3K9me3_peak_how_H3K27me3_change.pdf",p,width = 10,height = 6)
