...
diff_expression_analysis <- function(tissue){
  ...
  if(tissue %in% c("mammarygland","uterus","ovary","MEF","MEF_EZH2_inhibit")){
    tab <- tab[!grepl("chrY", tab$Chr), ]
  }
  rownames(tab) <- tab$Geneid
  tab <- tab[,-1]
  colnames <- colnames(tab)[6:length(tab)]
  pattern <- ".*bam\\.(LLX[0-9]+|CKJ[0-9]+|HM[0-9]+|TK[0-9]+|XM[0-9]+).*"
  ...
  # group <- group[which(group$TissueName==tissue),]
  age <- group$Age
