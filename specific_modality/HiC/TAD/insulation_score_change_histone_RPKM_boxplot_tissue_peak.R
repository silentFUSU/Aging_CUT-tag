...
for(antibody in antibodys){
  for(tissue in tissues){
    if(antibody == "ATAC"){
      ...
      tab <- read.table(paste0("data/samples/ATAC/",tissue,"/ATAC/",tissue,"_20000_redundant_tads.counts"),header = T) 
      tab_summary <- read.table(paste0("data/samples/ATAC/",tissue,"/ATAC/",tissue,"_20000_redundant_tads.counts.summary"),header=T)
    }else{
      search_table <- read.csv("data/samples/all/CUTTag_search_table_used_in_diff_batch.csv")
      tab <- read.table(paste0("data/samples/",tissue,"/",antibody,"/",tissue,"_20000_redundant_tads.counts"),header = T)
      tab_summary <- read.table(paste0("data/samples/",tissue,"/",antibody,"/",tissue,"_20000_redundant_tads.counts"),header = T)
    }
...
for(condition in c("Up","Stable","Down")){
  ...
  p <- ggplot(to_plot, aes(x = antibody, y = rowmeans,fill=age)) +
    ...
    ggtitle(condition)+
    ylim(0,2)
  # ggsave(paste0("result/Sup_figures/TAD_",condition,"_histone_ATAC_change.pdf"),p,width = 8,height = 6)
