...
tissue=$1
res=50000
mkdir -p ${data_path}${tissue}/compartment/homer_compartment
mkdir -p ${data_path}${tissue}/compartment/homer_compartment/allValidPair_homer
mkdir -p ${data_path}${tissue}/compartment/homer_compartment/tagDir
mkdir -p ${data_path}${tissue}/compartment/homer_compartment/PC1
...
do
    ...
    cut -d $'\t' -f 1-7 ${valid_pairs} > ${data_path}${tissue}/compartment/homer_compartment/allValidPair_homer/${sample}.allValidPairs.homer
    makeTagDirectory ${data_path}${tissue}/compartment/homer_compartment/tagDir/${sample} -format HiCsummary \
        ${data_path}${tissue}/compartment/homer_compartment/allValidPair_homer/${sample}.allValidPairs.homer
    
    runHiCpca.pl ${data_path}${tissue}/compartment/homer_compartment/PC1/${sample}_${res} ${data_path}${tissue}/compartment/homer_compartment/tagDir/${sample} -res ${res} -cpu 1 -genome mm10
done
