...
interaction_change_in_histone_condition_peak_level_logFC <- function(tissue,resolution){
  ...
  if(resolution == "200000"){
    ...
    write.csv(median_values,paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_recursion_peak_level_logFC.csv"),row.names = F)
  }else{
    mean_values <- to_plot %>%
      filter(is.finite(log2_ratio)) %>%
      group_by(combined_condition) %>%
      summarise(mean_log2_ratio = mean(log2_ratio, na.rm = TRUE))
    write.csv(mean_values,paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_recursion_peak_level_logFC.csv"),row.names = F)
  }
...
ggsave(paste0("result/HiC/all_tissues_",resolution,"_interaction_change_in_",antibody,"_histone_condition_recursion_peak_level_logFC.png"),combined_plot,width = 18,height = 20,type="cairo")

summary <- data.frame()
for(tissue in tissues){
  df <- read.csv(paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_recursion_peak_level_logFC.csv"))
...
if(resolution=="200000"){
  ...
  to_plot <- reshape2::dcast(summary, tissue ~ combined_condition, value.var = "mean_log2_ratio")
}
...
# breaks <- c(seq(-1, -0.31, length.out = 40), seq(-0.3, 0.3, length.out = 20), seq(0.31, 1, length.out = 40))
