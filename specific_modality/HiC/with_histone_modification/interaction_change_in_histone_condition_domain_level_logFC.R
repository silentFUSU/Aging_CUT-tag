...
interaction_change_in_histone_condition_peak_level_logFC <- function(tissue,resolution){
  ...
  for(sample in HiC_search_table$sample_name){
    ...
    if(resolution == "200000"){
      ...
      df <- df %>%
        dplyr::filter(abs(V1 - V2) >= 100 & abs(V1 - V2) <= 4000)
    }else{
  ...
  # conditions <- c("out-out","within-out","within-within")
  # p_value_summary <- data.frame()
  # for(i in c(1:2)){
  #   for(j in c((i+1):3)){
  #     p <- t.test(to_plot$log2_ratio[which(to_plot$combined_condition==conditions[i])],
  #            to_plot$log2_ratio[which(to_plot$combined_condition==conditions[j])])  
  #     t_p_value_summary <- data.frame(condition1=conditions[i],condition2=conditions[j],p_value=p$p.value)
  ...
  #   mutate(
  #   p_value = sapply(p_value, mark_significance)
  # )
  ...
  write.csv(median_values,paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_domain_level_logFC.csv"),row.names = F)
  ...
  # geom_signif(
  #   comparisons = list(c("out-out", "within-out"), c("within-out", "within-within"), c("within-within","out-out")),
  #   map_signif_level = TRUE,
  #   textsize = 4,test = "wilcox.test",
  #   y_position = c(-0.5, 0, 0.5),
  #   tip_length = c(1/500, 1/500, 1/500) 
  # )
  return(p)
}
...
ggsave(paste0("result/HiC/all_tissues_",resolution,"_interaction_change_in_",antibody,"_histone_condition_domain_level_logFC.png"),combined_plot,width = 18,height = 20,type="cairo")

summary <- data.frame()
for(tissue in tissues){
  df <- read.csv(paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_domain_level_logFC.csv"))
  df$tissue <- tissue_label_change(tissue)
  summary <- rbind(summary,df)
}
to_plot <- reshape2::dcast(summary, tissue ~ combined_condition, value.var = "median_log2_ratio")
...
pheatmap::pheatmap(to_plot,cluster_cols = F,breaks = breaks,color = color_palette)
