...
tissues <- c("brain","CB","kidney","liver","lung","bonemarrow","colon","heart","Hip","mammarygland","stomach","thymus","skin","muscle","cecum","ileum")
resolution <- "10000"
antibody <- "H3K9me3"
category_order <- c("kmeans1", "kmeans2", "kmeans3", "kmeans4","Stable","out")
category_map <- setNames(seq_along(category_order), category_order)
precomputed_conditions <- as.data.table(expand.grid(category_order, category_order))
precomputed_conditions[, combined := {
  # 获取映射并确保不含 NA
  idx <- category_map[c(Var1, Var2)]
  sorted_conditions <- c(Var1, Var2)[order(idx)]
  paste(sorted_conditions, collapse = "-")
}, by = .(Var1, Var2)]
conditions <- unique(precomputed_conditions$combined)
p_list <- list()
for(condition in conditions){
  ...
  for(tissue in tissues){
    ...
    df <- read.csv(paste0("result/HiC/",tissue,"/with_histone/H3K9me3_recursion_peaks/",resolution,"_interaction_relationship_with_",antibody,"_recursion_peaks_distance.csv"))  
  ...
  if(condition !="kmeans1-kmeans1"){
    to_plot <- to_plot[,tissue_order]
    p_list[[condition]] <- pheatmap::pheatmap(to_plot,cluster_rows = F,cluster_cols = F,breaks = breaks, color = color_palette,clustering_distance_cols = "manhattan",main = paste0(antibody," ",condition))
  }else{
    p_list[[condition]] <- pheatmap::pheatmap(to_plot,cluster_rows = F,breaks = breaks, color = color_palette,clustering_distance_cols = "manhattan",main = paste0(antibody," ",condition))
