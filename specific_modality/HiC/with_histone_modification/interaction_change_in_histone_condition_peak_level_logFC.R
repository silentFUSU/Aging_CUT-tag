  ...
  for(sample in HiC_search_table$sample_name){
    ...
    if(resolution == "200000"){
      df <- df %>%
        ...
        dplyr::filter(abs(V1 - V2) >= 5 & abs(V1 - V2) <= 400)
...
ggsave(paste0("result/HiC/all_tissues_",resolution,"_interaction_change_in_",antibody,"_histone_condition_peak_level_logFC.png"),combined_plot,width = 18,height = 20,type="cairo")

summary <- data.frame()
for(tissue in tissues){
  df <- read.csv(paste0("result/HiC/",tissue,"/differential_analysis/with_histone/",tissue,"_",resolution,"_interaction_change_in_",antibody,"_histone_condition_peak_level_logFC.csv"))
