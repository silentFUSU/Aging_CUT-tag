...
do
    mkdir -p ${data_path}${tissue}/loop/HiCCUPS/${sample}_${resolution}_optimal_parameter
    juicer_file=${data_path}${tissue}/juicer/${sample}.allValidPairs.hic
    for chr in ${chromosomes[@]}
    do
        mkdir -p ${data_path}${tissue}/loop/HiCCUPS/${sample}_${resolution}_optimal_parameter/${chr}
        ...
        ${juiceDir}/scripts/common/juicer_tools hiccups --cpu -r ${resolution} -k ${method} -f 0.1 -p 4 -i 15 -t 0.4,1.5,1.75,2 -d 40000 -c ${chr} ${juicer_file} ${data_path}${tissue}/loop/HiCCUPS/${sample}_${resolution}_optimal_parameter/${chr}
    done
done