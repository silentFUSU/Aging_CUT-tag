...
for(tissue in tissues){
  df <- read.table(paste0("data/samples/",tissue,"/H3K27me3/bed/H3K27me3_young_old_merge-W5000-G10000-E100.bed"))
  df$length <- df$V3 - df$V2 +1
  df <- df[which(df$length < 50000),]
  peak <- c(peak,nrow(df))    
}
peak <- median(peak)
peak_num <- 2500
...
for(tissue in tissues){
  ...
  # rpkm_mean_summary <- rpkm_mean_summary[which(rpkm_mean_summary$log2FC < 0),]
  rpkm_mean_summary <- rpkm_mean_summary[order(rpkm_mean_summary$log2FC),]  
  rpkm_mean_summary <- rpkm_mean_summary[c(1:peak_num),]
  t_rpkm_log2FC_summary <- data.frame(tissue=tissue_label_change(tissue),logFC = mean(rpkm_mean_summary$log2FC))
  rpkm_log2FC_summary <- rbind(rpkm_log2FC_summary,t_rpkm_log2FC_summary)
}

# log2FC_summary <- data.frame()
# for(tissue in tissues){
...
#   df <- df[order(df$LogFC.old.young),]
#   df <- df[c(1:peak_num),]
#   t_log2FC_summary <- data.frame(tissue=tissue_label_change(tissue),logFC = mean(df$LogFC.old.young))
#   log2FC_summary <- rbind(log2FC_summary,t_log2FC_summary)
# }
...
Indicator <- "logFC"
summary_antibody <- rpkm_log2FC_summary[,c("tissue",Indicator)]
...
emapplot(positive_gsea, showCategory = 30) 
emapplot(negative_gsea, showCategory = 30) 
...
ggplot(average_scores,aes(x=histone,y=score))+    
  geom_jitter(size = 3, alpha = 0.7,color="gray")+
  ...
  xlab("H3K27m3 log2(Fold change)")+labs(fill = "", color = "")+
  scale_x_reverse()
