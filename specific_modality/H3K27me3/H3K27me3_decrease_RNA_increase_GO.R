...
for(tissue in tissues){
  ...
  if(nrow(fourth_quadrant) >= 100){
    ...
    write.csv(result,paste0("data/samples/RNA/",tissue,"/H3K27me3_decreased_gene_expression_increased_GO_in_H3K27me3_peaks.csv"))
    ...
    ggsave(paste0("result/RNA/",tissue,"/H3K27me3_decreased_in_TSS_gene_expression_increased_GO_in_H3K27me3_peaks.png"),p,width=12,height=5,type="cairo")
  }
...
for(tissue in tissues){
  ...
  fourth_quadrant <- df[which(df$LogFC.old.young < 0 & df$logFC > 0),]
  if(nrow(fourth_quadrant) >=100){
    df <- read.csv(paste0("data/samples/RNA/",tissue,"/H3K27me3_decreased_gene_expression_increased_GO_in_H3K27me3_peaks.csv"))
