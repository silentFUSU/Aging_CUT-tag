...
tissue <- "lung"
state_num <- "11"
age_domain_chromHMM_annotation <- function(tissue,state_num){
  histone <- read.table(paste0("data/samples/",tissue,"/H3K27me3/peaks/edd/edd_peaks_fdr05.bed"))
  ...
  p <- ggplot(to_plot, aes(axis1 = Young_state, axis2 = Old_state, y = freq)) +  
    ...
    labs(y = "Count", x = "State Transition", 
         fill = "Young State")+
    ggtitle(paste0(tissue_label_change(tissue)," H3K27me3 age-domains"),"Sankey Plot of State Transitions")
  return(p)
}
...
for(tissue in tissues){
  p_list[[tissue]] <- age_domain_chromHMM_annotation(tissue,state_num)
}
combined_plot <- plot_a_list(p_list, 4, 7)
ggsave(paste0("result/all/diff/H3K27me3/H3K27me3_age_domain_chromHMM_annotation.png"),combined_plot,height = 20,width = 30,type="cairo")
