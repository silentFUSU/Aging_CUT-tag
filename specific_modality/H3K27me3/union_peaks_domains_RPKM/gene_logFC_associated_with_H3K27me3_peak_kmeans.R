...
p <- pheatmap::pheatmap(to_plot[,medians$tissues],show_rownames = F,cluster_cols=F,breaks = breaks, color = color_palette, clustering_distance_cols="manhattan",clustering_distance_rows="manhattan")
...
if(Indicator=="rank"){
  ...
  ggplot(to_plot,aes(x=histone,y=value,color =variable))+    
    ...
    ggtitle(paste0("Gene logFC positively correlated with the degree of downregulation of ",antibody))+
    theme_bw()+theme(text = element_text(size = 18),axis.text.x = element_text(angle = 45, hjust = 1))+
    xlab(paste0(antibody," log2(Fold change)"))+labs(fill = "", color = "") +ylab(paste0("Gene expression log2(Fold Change)"))+ scale_x_reverse()
}
