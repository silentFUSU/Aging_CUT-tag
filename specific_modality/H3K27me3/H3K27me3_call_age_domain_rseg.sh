...
mkdir -p  ${data_path}${tissue}/${antibody}/peaks/rseg/
genome_size=/storage/zhangyanxiaoLab/suzhuojie/ref_data/for_normal_mapping/mm10/mm10.chrom.sizes

rseg -c $genome_size -i 20 -v -o ${data_path}${tissue}/${antibody}/peaks/rseg/young.merge.domain.bed \
     -score ${data_path}${tissue}/${antibody}/peaks/rseg/young.merge.domain.score.wig \
     -boundary ${data_path}${tissue}/${antibody}/peaks/rseg/young.merge.domain.boundary.bed \
     -boundary-score ${data_path}${tissue}/${antibody}/peaks/rseg/young.merge.domain.boundary_score.wig \
     -bin-size 10000 -domain-size 200000 -bam ${data_path}${tissue}/${antibody}/tmp.young.merge.bam 