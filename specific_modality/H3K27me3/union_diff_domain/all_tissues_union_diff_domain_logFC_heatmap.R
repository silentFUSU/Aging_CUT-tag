...
antibody <- "H3K27me3"
common_increase <- read.csv(paste0("data/samples/all/",antibody,"/common_increase_edd_domain_merged_after_remove_batch_effect.csv"))
common_increase <- common_increase[which((common_increase$end - common_increase$start +1) > 1000000),]
common_decrease <- read.csv(paste0("data/samples/all/",antibody,"/common_decrease_edd_domain_merged_after_remove_batch_effect.csv"))
common_decrease <- common_decrease[which((common_decrease$end - common_decrease$start +1) > 1000000),]
...
annotation_color <- list(cluster=setNames(c("#F8766D", "#7CAE00", "#00BFC4"),c(1:3)))
...
write.csv(annotation,"data/samples/all/H3K27me3/edd_domain_merged/kmeans_annotation.csv")
...
for(kmean in c(1:k)){
  write.table(annotation_df[which(annotation_df$cluster==kmean),c(1:3)],paste0("data/samples/all/H3K27me3/edd_domain_merged/kmeans",kmean,"_edd_domain_merged.bed"),col.names = F,row.names = F,append = F,quote = F,sep = "\t")
}


#### only consider increased
common_increase <- read.csv(paste0("data/samples/all/",antibody,"/common_increase_edd_domain_merged_after_remove_batch_effect.csv"))
regions <- unique(c(common_increase$Geneid[which(common_increase$n>=10)]))
diff_summary <- data.frame()
