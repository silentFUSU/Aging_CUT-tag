...
antibody <- "H3K27me3"
kmeans <- "kmeans2"
regions <- read.table(paste0("data/samples/all/H3K27me3/peaks_merged_exist_in_27_tissues_edger/bed/",kmeans,"_uinon_recursion_peaks.bed"))
...
p <- pheatmap::pheatmap(diff_summary,show_rownames = F,breaks = breaks, color = color_palette, clustering_distance_cols="manhattan",clustering_distance_rows="manhattan",annotation_row = annotation,annotation_colors = annotation_color)
...
p <- pheatmap::pheatmap(diff_summary[,medians$tissue],show_rownames = F,cluster_cols = F,breaks = breaks, color = color_palette, clustering_distance_cols="manhattan",clustering_distance_rows="manhattan",annotation_row = annotation,annotation_colors = annotation_color)
...
if(Indicator=="rank"){
  ...
  ggplot(to_plot,aes(x=histone,y=log2(value),color =variable))+    
    ...
    ggtitle(paste0("Gene expression positively correlated with the degree of downregulation of ",antibody))+
...
if(Indicator=="rank"){
  ...
  ggplot(to_plot,aes(x=histone,y=log2(value),color =variable))+    
    ...
    ggtitle(paste0("Gene expression negatively correlated with the degree of downregulation of ",antibody))+
    theme_bw()+theme(text = element_text(size = 18),axis.text.x = element_text(angle = 45, hjust = 1))+
    xlab(Indicator)+labs(fill = "", color = "") +ylab(paste0("log2(CPM)"))
}else{
  ggplot(to_plot,aes(x=histone,y=log2(value),color =variable))+    
...
if(Indicator=="rank"){
  ...
  ggplot(average_scores,aes(x=histone,y=score,color = tissue))+    
    geom_jitter( size = 3, alpha = 0.7)+
    scale_color_manual(values = color)+
    ggtitle(paste0(description," with H3K27me3"))+
    theme_bw()+theme(text = element_text(size = 18),axis.text.x = element_text(angle = 45, hjust = 1))+
