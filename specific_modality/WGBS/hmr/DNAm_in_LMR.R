...
p <- ggplot(to_plot, aes(x = tissue, y = young_methylation, fill = condition)) +  
  geom_bar(stat = 'identity') +   
  theme_bw() +   
  scale_fill_manual(values = color) +
  theme(axis.title.x = element_blank(), 
...
to_plot$tissue_condition <- c("medium")
to_plot$tissue_condition[1:5] <- "bottom"
to_plot$tissue_condition[23:27] <- "top"
p <- ggplot(to_plot, aes(x = tissue_condition, y = young_methylation, fill = tissue_condition)) +  
  geom_boxplot() +  
  theme_bw() +   
  theme(axis.title.x = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1,vjust = 0.1),
  ...
  guides(fill = FALSE) 
p
wilcox.test(to_plot$young_methylation[which(to_plot$tissue_condition=="bottom")],to_plot$young_methylation[which(to_plot$tissue_condition=="top")])
wilcox.test(to_plot$young_methylation[which(to_plot$tissue_condition=="medium")],to_plot$young_methylation[which(to_plot$tissue_condition=="top")])
