...
# write.csv(tissue_summary,"data/samples/WGBS/all_tissues_all_samples_DNA_methylation_in_top1000_hmr_change_in_high_EZH2_SUZ12_peak_q01_level_hmr.csv")
tissue_summary <- read.csv("data/samples/WGBS/all_tissues_all_samples_DNA_methylation_in_top1000_hmr_change_in_high_EZH2_SUZ12_peak_q01_level_hmr.csv")
...
to_plot <- merge(to_plot,search_table[,c("sample_name","tissue")],by.x="sample",by.y="sample_name")
to_plot$tissue <- factor(to_plot$tissue,levels = tissue_order)

to_plot$age <- factor(to_plot$age,levels=c("3M","24M"))
ggplot(to_plot, aes(x = tissue, y = methylation, color = age)) +
