...
tissue <- "lung"
state_num <- 11
DMR_chromHMM_state_annotation <- function(tissue, condition){
  DMR <- read.table(paste0("data/samples/WGBS/LMJ_DMR_bed/",tissue,"_",condition,".bed"))
  ...
  overlaps_young <- foverlaps(DMR, chromHMM_young, type = "any", nomatch = 0L)  
  overlaps_old <- foverlaps(DMR, chromHMM_old, type = "any", nomatch = 0L)  
  ...
  p <- ggplot(to_plot, aes(axis1 = Young_state, axis2 = Old_state, y = freq)) +  
    ...
    labs(y = "Count", x = "State Transition", 
         fill = "Young State")+
    ggtitle(paste0(tissue_label_change(tissue)," ",str_to_title(condition)," DMR"),"Sankey Plot of State Transitions")
  return(p)
}
...
for(tissue in tissues){
  p_list[[tissue]] <- DMR_chromHMM_state_annotation(tissue,condition)
}
combined_plot <- plot_a_list(p_list, 4, 7)
ggsave(paste0("result/WGBS/all_tissues_",condition,"_DMR_annotation.png"),combined_plot,height = 20,width = 30,type="cairo")
