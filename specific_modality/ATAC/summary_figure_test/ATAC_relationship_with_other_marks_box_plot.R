  ...
  for(condition in c("up","down")){
    if(nrow(sig_list[[condition]]) >= 20){
      ...
      for(sample in t_search_table$sample_name){
        ...
        result <- data.frame(sample=sample,age=t_search_table$age[which(t_search_table$sample_name==sample)],methylation=result$methylation)
        WGBS_tissue_summary[[condition]] <- rbind(WGBS_tissue_summary[[condition]],result)
      }  
    }
  }
}
WGBS_tissue_summary <- readRDS("data/samples/ATAC/all/ATAC/WGBS_in_ATAC_diff_macs_young_old_narrowpeak_summits_spm3_diff_after_remove_batch_effect.rds")
search_table <- read.csv("data/samples/all/WGBS_search_table.csv")

WGBS_mean_tissue_summary <- list()
for(condition in c("up","down")){
  # WGBS_tissue_summary[[condition]] <- merge(WGBS_tissue_summary[[condition]],search_table[,c("tissue","sample_name")],by.x="sample",by.y="sample_name")
  summary_df <- WGBS_tissue_summary[[condition]] %>%
    group_by(tissue, age) %>%
    summarise(value = mean(methylation, na.rm = TRUE))
  summary_df$tissue <- sapply(summary_df$tissue,tissue_label_change)
...
for(condition in c("up","down")){
  ...
  df <- merge(young,old,by="tissue")
  test <- t.test(df$methylation.x,df$methylation.y,paired=T)
...
for(condition in c("up","down")){
  ...
  ggsave(paste0("result/figures/ATAC_WGBS_change_boxplot_",condition,".pdf"),p,width = 3,height = 6)
}
